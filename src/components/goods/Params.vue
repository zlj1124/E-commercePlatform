...
export default {
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      if (res.meta.status !== 200) {
        return this.$http.error('参数列表获取失败！')
      }
    ...
    async removeParams(attrId) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('参数删除成功！')
      }
    ...
    async saveAttrVals(row) {
      ...
      // console.log(res)
      if (res.meta.status !== 200) {
        return this.$message.error('参数标签添加失败！')
      }
      return this.$message.success('参数标签添加成功！')
    },
    // 点击按钮显示文本输入框
    showInput(row) {
